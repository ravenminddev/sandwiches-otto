                ...
                <ul className="flex flex-col gap-3">
                    {days.map((day, i) => (
                        <li key={day.date} className="flex items-center gap-3">
                            ...
                            <span className="flex-1 min-w-0 truncate text-sm sm:text-base text-slate-700 dark:text-zinc-300 capitalize">{formatLongDate(day.date)}</span>
                            <span className="shrink-0 text-sm sm:text-base font-bold text-slate-800 dark:text-zinc-200">{formatCOP(day.total)}</span>
                        </li>
                    ))}
                </ul>
            )}
