...
export default function StatCard({ icon, label, value, tone = 'amber', delay = 0 }) {
    ...
    return (
        <div
            className="group animate-fade-in-up bg-white rounded-2xl border border-slate-200/80 p-4 sm:p-5 flex items-center gap-4 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-slate-200/60"
        ...
        >
            ...
            <div className="min-w-0">
                ...
                <p className="text-lg sm:text-2xl font-bold tracking-tight text-slate-800 truncate">{value}</p>
            </div>
        </div>
    );
}
